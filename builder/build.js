...
const domElements = fs.readFileSync(path.join(frontend, 'game-ui', 'dom-elements.js'), 'utf8');
...
const capitalRenderer = fs.readFileSync(path.join(frontend, 'game-ui', 'capital-renderer.js'), 'utf8');
...
const output = html
  ...
  .replace('<script src="game-ui/dom-elements.js"></script>', `<script>${domElements}</script>`)
  ...
  .replace('<script src="game-ui/capital-renderer.js"></script>', `<script>${capitalRenderer}</script>`)
  .replace('<script src="game-ui/webgl-renderer.js"></script>', `<script>${webglRenderer}</script>`)
  .replace('<script src="game-ui/camera-controller.js"></script>', `<script>${cameraController}</script>`)
